...
while different rows correspond to different time points.
...
only the columns 'time' and 'x' are given but the dynamics is 3D. 'dynamics3_full.csv' is
a 2D dynamical system. The columns are again 'time', 'x' and 'y'. In 'dynamics3_observables.csv'
the state is not given. Instead, columns are 7 randomly chosen observation functions f_i(x,y)
of the true state variables x and y. 
...
'molecule2' is another molecular dynamics example, where the coordinates of the 101
heavy atoms of the molecule are given.
